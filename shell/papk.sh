...
#安卓包product文件夹路径
prod_path=${project_path}/build_outputs/apk/
#Flutter打包生成的最初地址
...
if (($number == 0 )); then
  ...
  for(( i=0;i<${c_length};i++)) do
    ...
    flutter build apk --no-shrink --dart-define=APP_ENV=${channels[$i]}
    cp -R ${release_path}*.apk ${prod_path}
  done;
else
  echo "=============== 正在构建：${channels[$((number-1))]} 渠道包 ==============="
  flutter build apk --no-shrink --dart-define=APP_ENV=${channels[$((number-1))]}
  cp -R ${release_path}*.apk ${prod_path}
fi
